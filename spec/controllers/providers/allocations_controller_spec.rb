    ...
    shared_examples "allocation journey mode" do |journey_mode|
      ...
      let(:previous_allocations) do
        training_provider_and_allocations_request_types_matrix.filter_map do |provider, request_type|
          build(:allocation, :with_uplift, request_type, accredited_body: accredited_body, provider: provider, confirmed_number_of_places: 5) if provider.recruitment_cycle == previous_recruitment_cycle
        end
      end

      let(:current_allocations) do
        training_provider_and_allocations_request_types_matrix.filter_map do |provider, request_type|
          build(:allocation, :with_uplift, request_type, accredited_body: accredited_body, provider: provider, confirmed_number_of_places: 5) if provider.recruitment_cycle == current_recruitment_cycle
        end
      end
